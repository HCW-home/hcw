...
class AppointmentFhir:
    ...
    def to_representation(self):
        ...
        # Extract appointments list from data
        appointments_list = self.data.get('results', []) if isinstance(self.data, dict) else self.data

        for appointment_data in appointments_list:
            # Map status
            status_mapping = {
                "Draft": "pending",
                "Scheduled": "booked",
            ...
            appointments.append(fhir_appointment.model_dump())

        self.data['results'] = appointments
        return json.dumps(appointments, default=str)
        return appointments
        return fhir_appointment.model_dump_json()
        return json.dumps({"entry": appointments})
