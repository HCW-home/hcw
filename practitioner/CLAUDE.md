...
- `<app-appointment-panel>` - The whole appointments block: header with totals, list/calendar switch, upcoming/past filter, one card per appointment with participants, join, outcome and overflow menu. `[source]` scopes it to a consultation (`{kind:'consultation', consultationId}`) or a patient (`{kind:'patient', patientId}`). It owns loading and the actions that are plain API calls; creating and editing are emitted as `(createRequested)` / `(editRequested)` so the host drives its own form modal. Hosts can call `reload()`, `upsert()`, `remove()` and `highlight()` on it, and mirror the list via `(loaded)`. Used by consultation-detail and patient-detail.
- `<app-contact-picker>` - **The single field used everywhere a beneficiary, a recipient or a participant is picked** (consultation form, create-consultation modal, consultation detail, reminder form, participant modal). One search box replaces the former "existing user / temporary guest" tabs: matching accounts come up as you type, and anything unknown can be invited straight from the result list — the channel (e-mail link, SMS link, or a link handed over manually) is deduced from what was typed. Once picked, the selection shows as a card with its type badge and a folded "invitation details" panel (first name, last name, language, timezone). Implements ControlValueAccessor over the selected account pk, so it drops into existing `beneficiary_id` / `recipient_id` controls; a temporary contact leaves that value null and the host reads `buildGuestPayload()` (or `(selectionChange)`) to post `temporary_beneficiary` / `temporary_participant`. Key inputs: `[initialUser]`, `[meUser]`, `[allowGuests]`, `[allowCreateAccount]` (adds a "create a full patient record" entry opening `add-edit-patient`), `[showVisibility]`, `[backendErrors]`. Host API: `isValid()`, `markAllTouched()`, `buildGuestPayload()`, `currentUser()`, `isConsultationVisible()`, `reset()`.
